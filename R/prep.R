...
BP <- snpinfo[,2]
 ...
 jam.results <-  JAM(marginal.betas=BETA,trait.variance=Vy[j],X.ref=refG, model.space.priors = list("a"=1, "b"=length(BETA), "Variables"=names(BETA)), n=N[j],xtx.ridge.term=0,save.path=save.path)
 } else {
   refG <- Gmat
   BETA <- beta1[[j]][colnames(refG)] # extract beta for SNPs in refG
  jam.results <-  JAM(marginal.betas=BETA,trait.variance=Vy[j],X.ref=refG, model.space.priors = list("a"=1, "b"=length(BETA), "Variables"=names(BETA)), n=N[j],xtx.ridge.term=0.01,save.path=save.path)
}
...
#' @author Jenn Asimit
#' @export
summary.stats <- function(Xmat=TRUE,ybar.all,main.input) {
